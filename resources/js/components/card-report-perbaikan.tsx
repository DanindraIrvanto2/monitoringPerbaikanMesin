...
import { Calendar, FileSpreadsheet, FileText, Wrench } from "lucide-react";
                ...
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                            Ekspor catatan servis, tindakan, & penggantian suku cadang
                        </p>
                    </div>
                </div>
            </CardHeader>
            <CardContent className="pt-4 space-y-4">
                <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                        <Label htmlFor="month-perbaikan" className="text-xs font-mono font-medium text-foreground flex items-center gap-1">
                            <Calendar className="size-3 text-muted-foreground" />
                            BULAN
                        </Label>
                        <Select value={data.month} onValueChange={handleChangeMonth} required>
                            <SelectTrigger id="month-perbaikan" className="w-full h-9 bg-muted/20 border-border text-xs font-mono">
                    ...
                    <div className="space-y-1.5">
                        <Label htmlFor="year-perbaikan" className="text-xs font-mono font-medium text-foreground flex items-center gap-1">
                            <Calendar className="size-3 text-muted-foreground" />
                            TAHUN
                        </Label>
                        <Select value={data.year} onValueChange={handleChangeYear} required>
                            <SelectTrigger id="year-perbaikan" className="w-full h-9 bg-muted/20 border-border text-xs font-mono">
